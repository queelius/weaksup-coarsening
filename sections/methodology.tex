...
$\hat{\bm{a}}$ should satisfy $\|\hat{\bm{a}} - \bm{a}\|_2 \le c_{\mathrm{tol}}\,
...
to total accuracy $\|\hat{\bm{a}} - \bm{a}\|_2 \le c_{\mathrm{tol}}\,\mathrm{gap}$
...
$(c_{\mathrm{tol}}\,\mathrm{gap})^2$ gives \eqref{eq:goldset-bound}. No $\log r$
...
floor $\ge \sigma_0^2 = (1 - \mathrm{gap}^2)/\beta_{\max} > 0$ and $n_g$
samples inside the problem, so its minimax risk lower-bounds the full
risk. The minimax L2 squared risk of the $r$-dimensional Gaussian
location model is $\inf_{\hat{\bm{a}}}\sup_{\bm{a}} \E\|\hat{\bm{a}} -
\bm{a}\|_2^2 \ge \sigma_0^2\, r/n_g$ (the Bayes risk under an isotropic
$N(0, \tau^2 I_r)$ prior tends to $\sigma_0^2 r/n_g$ as $\tau \to \infty$
and lower-bounds the minimax risk; equivalently the Cram\'er--Rao bound
with diagonal Fisher information $n_g/\sigma_0^2$ per coordinate,
\citealp{vandervaart1998asymptotic}).
This is information-theoretic, hence binds every estimator, not just the
hybrid one. Forcing the supremum risk below $(c_{\mathrm{tol}}\,\mathrm{gap})^2$
requires $n_g \ge (\sigma_0^2/c_{\mathrm{tol}}^2)\, r/\mathrm{gap}^2$, which is
...
$\|\hat{\bm{a}} - \bm{a}\|_2 \le c_{\mathrm{tol}}\,\mathrm{gap}$, the rate is
...
\bm{a}\|_2/\sqrt{r} \le c_{\mathrm{tol}}\,\mathrm{gap}$, the $r$ in the trace bound
...
This dependent-case budget is the load-bearing external contribution
of the paper relative to the candidate-set and weak-supervision
literature: the conditionally independent corner ($r = 0$) is the
regime of data programming and the triplet method, the gold-free
identifiability-up-to-swapping result is \citet{yu2022nplm}, and the
$\Theta(r/\mathrm{gap}^2)$ bound prices the gold data that restores a
correlated ensemble. Both factors are proved
(\cref{thm:goldset}) and confirmed in simulation
(\cref{sec:validation}): the gap sweep recovers the $1/\mathrm{gap}^2$
factor (log--log slope $-2.04$) and the $r$-sweep recovers the
linear-in-$r$ rate for total model recovery (log--log slope $1.17$,
tracking the trace-bound theory line), together with the
$\log r$ and $r$-independent rates that \cref{rmk:r-dependence} assigns
to the weaker per-direction and per-coordinate criteria.
