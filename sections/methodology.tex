...
correlated ensemble. The two factors carry different evidentiary
weight: the $1/\mathrm{gap}^2$ factor is both proved
(\cref{thm:goldset}) and confirmed in simulation
(\cref{sec:validation}), whereas the linear-in-$r$ rate is proved here
but not yet validated by an $r$-sweep, which the longer manuscript
supplies \citep{towell2026weaksupcoarseningextended}.
