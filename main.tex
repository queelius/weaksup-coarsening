...
is the load-bearing external contribution, and both factors of the
$\Theta(r/\mathrm{gap}^2)$ rate are confirmed: the empirical
$\log$--$\log$ slope of $-2.04$ for the gold-set size against the
accuracy margin matches the predicted $1/\mathrm{gap}^2$ factor, and an
$r$-sweep at fixed margin recovers the linear-in-$r$ rate for total
model recovery (slope $1.17$). The framework casts the
